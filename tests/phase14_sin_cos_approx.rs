...
mod tests {
    ...
    /// Compile a kernel that runs `intrinsic` over `inputs` lane-by-lane,
    /// link with C harness that calls `ref_fn`, assert absolute error ≤ 3e-6.
    ///
    /// Absolute error tolerance because sin/cos pass through zero at the
    /// quadrant boundaries — relative error blows up there.
