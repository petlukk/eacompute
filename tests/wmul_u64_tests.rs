//! `wmul_u64_lo` / `wmul_u64_hi` — cross-platform u32×u32 → u64 widening
//! multiply (Poly1305 unblocker).
//!
//! Each call widens **two** logical lanes:
//!   - `wmul_u64_lo(u32x4, u32x4) -> u64x2` operates on lanes 0,1
//!   - `wmul_u64_hi(u32x4, u32x4) -> u64x2` operates on lanes 2,3
//!
//! Pair them to widen all four lanes of a u32x4. Numeric tests use values
//! near the u32 boundary so a 32-bit truncation would be visible.
//!
//! ARM is verified by cross-compile to object; numeric x86 tests use the
//! host runner. Same kernels both targets — exact lane semantics are
//! contract.
