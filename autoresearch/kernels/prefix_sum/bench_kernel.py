#!/usr/bin/env python3
"""Compile and benchmark an Eä prefix sum kernel. Outputs one JSON line to stdout.
...
Key difference from FMA: prefix sum has an inherent sequential dependency.
Correctness uses relaxed tolerance (rtol=1e-2) due to FP accumulation error.
"""
...
BYTES_PER_ELEM = 8  # read in + write out
NUM_RUNS = 100
WARMUP_RUNS = 10
SEED = 42
...
I32 = ctypes.c_int32
PREFIX_SUM_ARGTYPES = [FLOAT_PTR, FLOAT_PTR_MUT, I32]
...
def bench_at_size(ea_func, ref_func, size):
    ...
    # Correctness check
    ref_func(dp, ref_op, n)
    ea_func(dp, ea_op, n)

    if not np.allclose(ea_out, ref_out, rtol=1e-2):
...
def main():
    ...
    try:
        ea_func = ea_lib.prefix_sum_f32
        ea_func.argtypes = PREFIX_SUM_ARGTYPES
        ea_func.restype = None

        ref_func = ref_lib.prefix_sum_f32_c
        ref_func.argtypes = PREFIX_SUM_ARGTYPES
