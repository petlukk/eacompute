#!/usr/bin/env python3
"""Compile and benchmark an Eä dot product kernel. Outputs one JSON line to stdout.
...
Dot product: result = sum(a[i] * b[i]). Returns scalar float.
Correctness uses relaxed tolerance (rtol=1e-2) due to FP associativity.
"""
...
BYTES_PER_ELEM = 8  # read a + read b
NUM_RUNS = 100
WARMUP_RUNS = 10
SEED = 42
...
I32 = ctypes.c_int32
DOT_ARGTYPES = [FLOAT_PTR, FLOAT_PTR, I32]
...
def bench_at_size(ea_func, ref_func, size):
    ...
    # Correctness check
    ref_result = ref_func(ap, bp, n)
    ea_result = ea_func(ap, bp, n)
