#!/usr/bin/env python3
"""Compile and benchmark an Eä batch cosine similarity kernel. Outputs JSON to stdout.

Runs across multiple (dim, n_vecs) configs to prevent overfitting to a single
cache/memory behavior. The reported time_us is the median across all configs.

Correctness uses rtol=1e-3 due to FP associativity and sqrt differences.
"""
...
I32 = ctypes.c_int32
COSINE_ARGTYPES = [FLOAT_PTR, FLOAT_PTR, I32, I32, FLOAT_PTR]
...
def main():
    ...
    try:
        ea_func = ea_lib.batch_cosine
        ea_func.argtypes = COSINE_ARGTYPES
        ea_func.restype = None

        ref_func = ref_lib.batch_cosine_ref
        ref_func.argtypes = COSINE_ARGTYPES
