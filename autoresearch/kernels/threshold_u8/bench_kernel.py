#!/usr/bin/env python3
"""Compile and benchmark Ea threshold_u8x16 kernel. Outputs JSON to stdout.

Benchmarks threshold_u8x16 across real image sizes.
"""
...
BYTES_PER_ELEM = 2  # read u8 + write u8
NUM_RUNS = 50
WARMUP_RUNS = 10
SEED = 42
THRESH = 128
...
def bench_at_size(ea_func, ref_func, n):
    ...
    ea_dst = np.zeros(n, dtype=np.uint8)
    ref_dst = np.zeros(n, dtype=np.uint8)

    src_p = src.ctypes.data_as(U8_PTR)
    ea_dst_p = ea_dst.ctypes.data_as(U8_PTR)
    ref_dst_p = ref_dst.ctypes.data_as(U8_PTR)

    ea_func(src_p, ea_dst_p, I32(n), U8(THRESH))
    ref_func(src_p, ref_dst_p, I32(n), U8(THRESH))

    if not np.array_equal(ea_dst, ref_dst):
        mismatches = np.where(ea_dst != ref_dst)[0]
        first = mismatches[0]
        return (f"correctness: mismatch at index {first}: "
                f"ea={ea_dst[first]} ref={ref_dst[first]} src={src[first]} (N={n})")
...
def main():
    ...
    try:
        ea_func = ea_lib.threshold_u8x16
        ea_func.argtypes = [U8_PTR, U8_PTR, I32, U8]
        ea_func.restype = None

        ref_func = ref_lib.threshold_u8x16_ref
        ref_func.argtypes = [U8_PTR, U8_PTR, I32, U8]
