#!/usr/bin/env python3
"""Compile and benchmark an Eä horizontal reduction kernel. Outputs one JSON line to stdout.
...
Key difference from FMA: sum_f32x8 returns a scalar float, so correctness
uses relaxed tolerance (rtol=1e-2) due to FP associativity differences.
"""
...
BYTES_PER_ELEM = 4  # read data only
NUM_RUNS = 100
WARMUP_RUNS = 10
SEED = 42
...
def bench_at_size(ea_func, ref_func, size):
    ...
    np.random.seed(SEED)
    data = np.random.uniform(-1, 1, size).astype(np.float32)

    dp = data.ctypes.data_as(FLOAT_PTR)
    n = I32(size)

    # Correctness check
    ref_result = ref_func(dp, n)
    ea_result = ea_func(dp, n)

    if not np.isclose(ea_result, ref_result, rtol=1e-2):
        return f"correctness: ea={ea_result:.6f} ref={ref_result:.6f} diff={abs(ea_result - ref_result):.6f} (size={size})"
...
def main():
    ...
    try:
        ea_func = ea_lib.sum_f32x8
        ea_func.argtypes = SUM_ARGTYPES
        ea_func.restype = ctypes.c_float

        ref_func = ref_lib.sum_f32x8_c
        ref_func.argtypes = SUM_ARGTYPES
        ref_func.restype = ctypes.c_float
