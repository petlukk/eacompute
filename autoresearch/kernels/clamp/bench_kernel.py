#!/usr/bin/env python3
"""Compile and benchmark an Eä clamp kernel. Outputs one JSON line to stdout.

Clamp: result[i] = clamp(data[i], lo, hi). Tests select/masking codegen.
Signature: clamp_f32x8(data: *f32, result: *mut f32, lo: f32, hi: f32, len: i32) -> void
"""
...
BYTES_PER_ELEM = 8  # read data + write result
NUM_RUNS = 100
WARMUP_RUNS = 10
SEED = 42
...
I32 = ctypes.c_int32
CLAMP_ARGTYPES = [FLOAT_PTR, FLOAT_PTR, ctypes.c_float, ctypes.c_float, I32]
...
def bench_at_size(ea_func, ref_func, size):
    ...
    np.random.seed(SEED)
    data = np.random.uniform(-2, 2, size).astype(np.float32)
    lo = np.float32(-0.5)
    hi = np.float32(0.5)

    dp = data.ctypes.data_as(FLOAT_PTR)

    # Correctness check
    ea_result = np.zeros(size, dtype=np.float32)
    ref_result = np.zeros(size, dtype=np.float32)

    ref_func(dp, ref_result.ctypes.data_as(FLOAT_PTR),
             ctypes.c_float(lo), ctypes.c_float(hi), I32(size))
    ea_func(dp, ea_result.ctypes.data_as(FLOAT_PTR),
            ctypes.c_float(lo), ctypes.c_float(hi), I32(size))
    ...
    # Benchmark
    bench_result = np.zeros(size, dtype=np.float32)
    rp = bench_result.ctypes.data_as(FLOAT_PTR)

    for _ in range(WARMUP_RUNS):
        ea_func(dp, rp, ctypes.c_float(lo), ctypes.c_float(hi), I32(size))
...
def main():
    ...
    try:
        ea_func = ea_lib.clamp_f32x8
        ea_func.argtypes = CLAMP_ARGTYPES
        ea_func.restype = None

        ref_func = ref_lib.clamp_f32x8_c
        ref_func.argtypes = CLAMP_ARGTYPES
