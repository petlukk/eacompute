...
- `exp_poly_f32` throughput vs scalar `exp()` is baseline-dependent. Measured on the Phase 6 benchmarks:
  ...
  - The spec's original "~10×" headline is an upper bound against older or scalar-only libm without a vectorized `expf`; against modern glibc the win is the 2-3× range shown above. See [`docs/release/v1.11.0/perf-results.md`](docs/release/v1.11.0/perf-results.md) for the methodology and full numbers.
